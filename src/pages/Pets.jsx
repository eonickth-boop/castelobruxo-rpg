...
const falasPorEspecie = {
  'arara azul': [
    'O céu está bonito hoje. Vamos explorar?',
    'Ouvi algo interessante perto da floresta.',
    'Gosto quando você volta para me visitar.',
  ],
  'coruja das matas': [
    'A noite guarda segredos para quem sabe observar.',
    'A biblioteca parece especialmente silenciosa hoje.',
    'Conhecimento também é uma forma de proteção.',
  ],
  'jabuti ancestral': [
    'Nem todo caminho precisa ser percorrido depressa.',
    'As raízes antigas contam histórias importantes.',
    'Descansar também faz parte da jornada.',
  ],
  'lagarto esmeralda': [
    'Senti uma energia diferente perto dos cristais.',
    'Vamos procurar um lugar ensolarado?',
    'Algo brilhou ali. Você viu?',
  ],
  'raposa do mato': [
    'Tenho certeza de que existe um atalho por aqui.',
    'Hoje parece um bom dia para uma aventura.',
    'Eu ouvi passos, mas não eram os seus.',
  ],
  'sapo encantado': [
    'A umidade está perfeita para novos experimentos.',
    'Talvez exista algo raro perto do lago.',
    'Croac... isso significa que senti sua falta.',
  ],
  'tucano amazonico': [
    'A floresta parece diferente hoje.',
    'Ouvi algo perto da biblioteca...',
    'Vamos explorar?',
  ],
}
...
function falasDoPet(registro) {
  const especie = normalizar(registro?.pets?.especie || '')
  const chave = Object.keys(falasPorEspecie).find((item) =>
    especie.includes(item),
  )

  return falasPorEspecie[chave] || [
    'Que bom ver você novamente.',
    'Vamos descobrir algo novo hoje?',
...
export default function Pets({
  ...
  async function carregarDados() {
    ...
    const registros = respostaUsuario.data ?? []

    setCatalogo(respostaCatalogo.data ?? [])
    setMeusPets(registros)

    const ativo = registros.find((registro) => registro.equipado)
    if (ativo) {
      const opcoes = falasDoPet(ativo)
      setFala(opcoes[Math.floor(Math.random() * opcoes.length)])
    }
  ...
  function reagirAoClique(registro) {
    const opcoes = falasDoPet(registro)
    const novaFala = opcoes[Math.floor(Math.random() * opcoes.length)]
    setFala(novaFala)
  }
  ...
  return (
    <main className="pets-page">
      ...
      <header className="pets-hero">
        <p>Companheiros vivos</p>
        <h1>Meus Companheiros</h1>
        <span>
          Adote, cuide e crie vínculos com as criaturas de Castelobruxo.
        </span>
      ...
      {companheiroAtivo && (
        <section className="pet-ativo pet-ativo-vivo">
          <button
            type="button"
            className="pet-vivo-imagem pet-movimento-respirar"
            onClick={() => reagirAoClique(companheiroAtivo)}
            aria-label="Interagir com o companheiro"
          >
          ...
          <div className="pet-ativo-dados">
            ...
            <span>
              {companheiroAtivo.pets?.especie} ·{' '}
              {companheiroAtivo.pets?.raridade || 'Comum'} ·{' '}
              Humor: {companheiroAtivo.humor || 'curioso'}
            </span>

            {fala && (
              <blockquote className="pet-fala">
                “{fala}”
              </blockquote>
            )}
            ...
            <div className="pet-ativo-acoes">
              <button
                type="button"
                onClick={() => onAbrirPet(companheiroAtivo)}
              >
      ...
      {carregando ? (
        ...
        <p className="pets-vazio">
          Nenhum pet encontrado neste filtro.
        </p>
      ) : (
        <section className="pets-grade">
          {petsExibidos.map((pet) => {
            ...
            return (
              ...
              >
                <div className="pet-card-icone pet-card-imagem pet-movimento-suave">
                  {pet.imagem_url ? (
                ...
                <div className="pet-card-conteudo">
                  ...
                  {registro ? (
                    <div className="pet-card-estatisticas">
                      ...
                      <span>
                        Adotado em{' '}
                        {formatarData(registro.adquirido_em)}
                      </span>
                    </div>
                  ) : (
                    <div className="pet-card-estatisticas">
