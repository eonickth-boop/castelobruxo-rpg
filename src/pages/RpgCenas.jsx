...
export default function RpgCenas() {
  ...
  async function carregarCena(cena, sessaoFornecida = sessao, mostrarCarregamento = true) {
    if (mostrarCarregamento) setMensagem('')
    const usuarioId = sessaoFornecida?.user?.id

    const [{ data: posts, error: erroPosts }, { data: listaParticipantes, error: erroParticipantes }] = await Promise.all([
    ...
    if (erroPosts || erroParticipantes) {
      if (mostrarCarregamento) setMensagem('Não foi possível atualizar a cena.')
      return
    }

    const ids = [...new Set([...(listaParticipantes || []).map((item) => item.usuario_id), ...(posts || []).map((item) => item.autor_id)])]
    let mapaPerfis = {}
    if (ids.length) {
  ...
  async function sairDaCena() {
    ...
    const { error } = await supabase.from('rpg_participantes')
      .delete().eq('cena_id', cenaAtual.id).eq('usuario_id', sessao.user.id)
  ...
  async function publicar(evento) {
    ...
    const { data, error } = await supabase.from('rpg_postagens').insert({
      cena_id: cenaAtual.id, autor_id: sessao.user.id, tipo, conteudo: texto,
    }).select().single()
  ...
  async function salvarEdicao(postagem) {
    ...
    if (!texto) return
    const { data, error } = await supabase.from('rpg_postagens')
      .update({ conteudo: texto, editado_em: new Date().toISOString() })
      .eq('id', postagem.id).select().single()
    ...
    return (
      <main className="rpg-shell rpg-shell-cena">
        ...
        <header className="rpg-cabecalho-cena">
          <div>
            <span>{cenaAtual.local_nome}</span>
            <h1>{cenaAtual.titulo}</h1>
            <p>{cenaAtual.descricao || 'Uma nova história está começando.'}</p>
          </div>
          <div className="rpg-badges"><strong>{cenaAtual.status}</strong><small>{cenaAtual.visibilidade}</small><small>{cenaAtual.ordem_turnos === 'fixa' ? 'Turnos fixos' : 'Turnos livres'}</small></div>
        </header>

        <section className="rpg-resumo-cena">
          <div><small>Participantes</small><strong>{participantes.length}</strong></div>
          <div><small>Turnos publicados</small><strong>{postagens.length}</strong></div>
          <div><small>Último turno</small><strong>{ultimoPerfil ? nomeDoPerfil(ultimoPerfil) : 'Nenhum'}</strong></div>
          <div><small>Atualização</small><strong>Automática</strong></div>
        </section>

        <div className="rpg-layout-cena">
          <section className="rpg-coluna-principal">
            <article className="rpg-ambiente">
              <span>Descrição do ambiente</span>
              <p>{cenaAtual.descricao || 'O cenário ainda não recebeu uma descrição detalhada.'}</p>
            </article>
              ...
              {postagens.length === 0 ? <div className="rpg-vazio">Ainda não há turnos nesta cena.</div> : postagens.map((postagem) => {
                ...
                return (
                  <article key={postagem.id} className={`rpg-postagem rpg-${postagem.tipo}`}>
                    <div className="rpg-postagem-avatar">
                      {autor.avatar_url ? <img src={autor.avatar_url} alt={nomeDoPerfil(autor)} /> : <span>{nomeDoPerfil(autor).slice(0, 1).toUpperCase()}</span>}
                    </div>
                    <div className="rpg-postagem-corpo">
                      <header>
                        <div><strong>{nomeDoPerfil(autor)}</strong><small>{autor.tribo || 'Sem tribo'} · Nível {autor.nivel || 1} · {autor.cargo || 'aluno'}</small></div>
                        <div className="rpg-meta-turno"><span>{postagem.tipo}</span><time>{new Date(postagem.criado_em).toLocaleString('pt-BR')}</time></div>
                      </header>

                      {editandoId === postagem.id ? (
                        <div className="rpg-edicao-inline">
                          <textarea value={textoEdicao} onChange={(e) => setTextoEdicao(e.target.value)} rows={5} maxLength={5000} />
                          <div><button onClick={() => salvarEdicao(postagem)}>Salvar</button><button className="rpg-secundario" onClick={() => setEditandoId(null)}>Cancelar</button></div>
                        </div>
                      ) : <p>{postagem.conteudo}</p>}

                      {postagem.editado_em && <small className="rpg-editado">Editado em {new Date(postagem.editado_em).toLocaleString('pt-BR')}</small>}
                      {propria && editandoId !== postagem.id && (
                        <div className="rpg-acoes-postagem"><button onClick={() => { setEditandoId(postagem.id); setTextoEdicao(postagem.conteudo) }}>Editar</button><button onClick={() => excluirPostagem(postagem)}>Excluir</button></div>
                      )}
                    </div>
                  </article>
                )
            ...
            {cenaAtual.status === 'aberta' && participando && (
              <form className="rpg-editor" onSubmit={publicar}>
                <div><label>Tipo do turno</label><select value={tipo} onChange={(e) => setTipo(e.target.value)}><option value="acao">Ação</option><option value="fala">Fala</option><option value="pensamento">Pensamento</option>{podeNarrar && <option value="narracao">Narração</option>}</select></div>
                <textarea value={conteudo} onChange={(e) => setConteudo(e.target.value)} placeholder="Descreva o turno do seu personagem..." maxLength={5000} rows={7} />
                {conteudo.trim() && <div className="rpg-preview"><small>Pré-visualização</small><p>{conteudo}</p></div>}
                <button type="submit">Publicar turno</button>
              </form>
            )}
            {cenaAtual.status === 'pausada' && <div className="rpg-vazio">A cena está pausada pelo narrador. Novos turnos estão temporariamente bloqueados.</div>}
          ...
          <aside className="rpg-painel-lateral">
            ...
            {podeNarrar && (
              <section className="rpg-controles-narrador">
                <h2>Painel do narrador</h2>
                {cenaAtual.status === 'aberta' && <button onClick={() => mudarStatus('pausada')}>Pausar cena</button>}
                {cenaAtual.status === 'pausada' && <button onClick={() => mudarStatus('aberta')}>Retomar cena</button>}
                {cenaAtual.status !== 'encerrada' && <button className="rpg-encerrar" onClick={() => mudarStatus('encerrada')}>Encerrar cena</button>}
              </section>
            )}
          </aside>
        </div>
    ...
    <main className="rpg-shell">
      ...
      {formAberto && (
        <form className="rpg-nova-cena" onSubmit={criarCena}>
          ...
          <textarea placeholder="Descrição inicial e ambiente" rows={5} value={novaCena.descricao} onChange={(e) => setNovaCena({ ...novaCena, descricao: e.target.value })} />
          ...
          <select value={novaCena.ordem_turnos} onChange={(e) => setNovaCena({ ...novaCena, ordem_turnos: e.target.value })}><option value="livre">Turnos livres</option><option value="fixa">Ordem fixa</option></select>
      ...
      <section className="rpg-grade-cenas">
        {cenasVisiveis.length === 0 ? <div className="rpg-vazio">Nenhuma cena encontrada neste filtro.</div> : cenasVisiveis.map((cena) => (
          <button key={cena.id} className="rpg-card-cena" onClick={() => carregarCena(cena)}>
