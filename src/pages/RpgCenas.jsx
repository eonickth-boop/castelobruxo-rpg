...
export default function RpgCenas() {
  ...
  const [novaCena, setNovaCena] = useState({ titulo: '', descricao: '', local_nome: '', visibilidade: 'publica' })

  const nomeJogador = useMemo(() => perfil?.nome_personagem || perfil?.usuario || 'Jogador', [perfil])
  ...
  async function abrirCena(cena) {
    ...
    const [{ data: posts }, { data: participante }] = await Promise.all([
      ...
      supabase.from('rpg_participantes').select('id').eq('cena_id', cena.id).eq('usuario_id', sessao.user.id).maybeSingle(),
    ])
  ...
  async function criarCena(evento) {
    ...
    setNovaCena({ titulo: '', descricao: '', local_nome: '', visibilidade: 'publica' })
    setFormAberto(false)
    abrirCena(data)
  }
  ...
  async function publicar(evento) {
    ...
    const { data, error } = await supabase
      .from('rpg_postagens')
      .insert({ cena_id: cenaAtual.id, autor_id: sessao.user.id, tipo, conteudo: texto })
      .select()
      .single()
  ...
  if (!sessao) {
    return (
      <main className="rpg-shell rpg-centralizado">
        <h1>RPG Textual</h1>
        <p>Entre na sua conta para participar das cenas.</p>
  ...
  if (cenaAtual) {
    ...
    return (
      <main className="rpg-shell">
        <button className="rpg-voltar" onClick={() => setCenaAtual(null)}>← Voltar às cenas</button>
        ...
        <header className="rpg-cabecalho-cena">
          ...
          <div className="rpg-badges">
            <strong>{cenaAtual.status}</strong>
            <small>{cenaAtual.visibilidade}</small>
          </div>
        </header>

        <section className="rpg-postagens">
          {postagens.length === 0 ? (
            <div className="rpg-vazio">Ainda não há turnos nesta cena.</div>
          ) : postagens.map((postagem) => (
            <article key={postagem.id} className={`rpg-postagem rpg-${postagem.tipo}`}>
              <header>
                <strong>{postagem.autor_id === sessao.user.id ? nomeJogador : 'Outro personagem'}</strong>
                <span>{postagem.tipo}</span>
                <time>{new Date(postagem.criado_em).toLocaleString('pt-BR')}</time>
              </header>
        ...
        {cenaAtual.status === 'aberta' && !participando && (
          <button className="rpg-entrar" onClick={entrarNaCena}>Entrar nesta cena</button>
        )}
          ...
          <form className="rpg-editor" onSubmit={publicar}>
            <div>
              ...
              <select value={tipo} onChange={(e) => setTipo(e.target.value)}>
                ...
                {podeNarrar && <option value="narracao">Narração</option>}
              </select>
            </div>
            <textarea
              value={conteudo}
              onChange={(e) => setConteudo(e.target.value)}
              placeholder="Descreva a ação, fala ou narração do seu personagem..."
              maxLength={5000}
              rows={7}
            />
        ...
        {podeNarrar && cenaAtual.status === 'aberta' && (
          <button className="rpg-encerrar" onClick={encerrarCena}>Encerrar cena</button>
        )}
    ...
    <main className="rpg-shell">
      <header className="rpg-topo">
        <div>
          ...
          <h1>Salão de Interpretação</h1>
          <p>Crie cenas, interprete seu personagem e continue a história pelo site.</p>
        </div>
        <div className="rpg-acoes-topo">
          <button onClick={() => setFormAberto((valor) => !valor)}>Nova cena</button>
          <button className="rpg-secundario" onClick={() => { window.location.href = '/' }}>Voltar ao portal</button>
        </div>
      </header>
      ...
      {formAberto && (
        <form className="rpg-nova-cena" onSubmit={criarCena}>
          ...
          <input placeholder="Local" value={novaCena.local_nome} onChange={(e) => setNovaCena({ ...novaCena, local_nome: e.target.value })} />
          ...
          <select value={novaCena.visibilidade} onChange={(e) => setNovaCena({ ...novaCena, visibilidade: e.target.value })}>
      ...
      <section className="rpg-grade-cenas">
        {cenas.length === 0 ? (
          <div className="rpg-vazio">Nenhuma cena foi criada ainda.</div>
        ) : cenas.map((cena) => (
          <button key={cena.id} className="rpg-card-cena" onClick={() => abrirCena(cena)}>
            <span>{cena.local_nome}</span>
            <h2>{cena.titulo}</h2>
            <p>{cena.descricao || 'Sem descrição.'}</p>
            <footer>
              <strong>{cena.status}</strong>
              <small>{new Date(cena.criado_em).toLocaleDateString('pt-BR')}</small>
            </footer>
          </button>
        ))}
      </section>
