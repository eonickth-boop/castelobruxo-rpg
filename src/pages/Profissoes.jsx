...
export default function Profissoes({perfil:perfilRecebido,onVoltar}){
  const [perfilInterno,setPerfilInterno]=useState(perfilRecebido||null)
  ...
  const perfil=perfilRecebido||perfilInterno
  ...
  async function carregar(){
    ...
    if(!session?.user){setMensagem('Entre na sua conta para acessar as profissões.');setCarregando(false);return}
    const {data:perfilAtual}=await supabase.from('perfis').select('id,usuario,nome_personagem,nivel,ano,tribo').eq('id',session.user.id).maybeSingle()
    if(perfilAtual)setPerfilInterno(perfilAtual)
    ...
    setProfissoes(lista||[]);setReputacao(rep||null);setAtual(registro||null);setCarregando(false)
  }
  ...
  async function abandonar(){
    if(!window.confirm('Deseja realmente abandonar sua profissão atual?'))return
    setAcao(true);setMensagem('')
  ...
  const voltar=onVoltar||(()=>{window.location.href='/'})

  return <main className="profissoes-pagina">
    <button className="profissoes-voltar" onClick={voltar}>← Voltar</button>
